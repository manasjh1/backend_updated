...
async def send_otp_route(data: schemas.Phone):
    ...
    if not TWOFACTOR_API_KEY:
        ...
        try:
            ...
            base_url = f"https://2factor.in/API/V1/{TWOFACTOR_API_KEY}/SMS/{mobile_number}/AUTOGEN/{otp}"
...
async def register_farmer_endpoint(data: schemas.FarmerCreate, db: Session = Depends(get_db)):
    ...
    if not stored_otp:
        ...
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str("OTP not found or has expired. Please request again."))
    if stored_otp != submitted_otp:
        log.warning(f"Registration Failure ({masked_mobile}): Invalid OTP submitted.")
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str("Invalid OTP provided."))
    ...
    try:
        ...
        if existing_mobile:
            ...
            raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail=str("This mobile number is already registered."))
        if data.aadhaar_number:
             ...
             if existing_aadhaar:
                  ...
                  raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail=str("This Aadhaar number is already registered."))
    except SQLAlchemyError as e:
         log.error(f"Database error checking existing user ({masked_mobile}): {e}", exc_info=True)
         raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=str("Error checking user registry."))
    ...
    try:
        ...
        db_data_to_save = {
            ...
            "crop_type": data.crop_type or None,
            "village": data.village or None,
            "district": data.district or None,
            "state": data.state or None,
            "pin_code": data.pin_code or None,
            "cultivation_unit": data.cultivation_unit or None,
            "approximate_produce": data.approximate_produce or None,
            "geo_location": data.geo_location or None,
    ...
    # --- Handle Database Errors During Commit ---
         ...
         raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail=str(detail))
         ...
         raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=str("Database error during registration."))
    except Exception as e: # Catch any other unexpected errors
         db.rollback()
         log.error(f"Unexpected Server Error during registration processing ({masked_mobile}): {e}", exc_info=True)
         raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=str("Unexpected server error during registration."))
